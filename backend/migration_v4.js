// migration_v4.js
// This script manually fetches categories to prevent crashes.
...
import Category from './models/categoryModel.js'; // We need this
...
const migrateOldProducts = async () => {
  try {
    ...
    for (const product of productsToMigrate) {
      // --- THIS IS THE FIX ---
      // Manually fetch the category using the ID
      let categoryName = '';
      try {
        ...
        if (category) {
          ...
          console.log(`Skipping: ${product.name} (Category ID not found)`);
          continue;
        }
      } catch (e) {
        console.log(`Skipping: ${product.name} (Error fetching category)`);
        continue;
      }
      // ---------------------
